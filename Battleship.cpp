...
void DisplayWinner(const Player& player1, const Player& player2)
{
    ...
    {
        cout<<"Congratlations! "<<player1.playerName<<"! You won!"<<endl;
    }
}

bool IsSunk(const Player& player, const Ship& ship) // z tym moze byc problem
{
...
void PlayGame(Player& player1, Player& player2)
{
    ...
    if(player2.playerType==PT_HUMAN)
    {
        cout<<"Press enter to clear the screen..."<<endl;
        cin.ignore();
        system("CLS");
        cout<<"switch the players and press enter to continue..."<<endl;
        cin.ignore();
    }
    else
    {
    ...
    do
    {
        ...
        if(currentPlayer->playerType == PT_AI)
        {
            ...
            cout<<currentPlayer->playerName<<" choose row "<<char(guess.row + 'A')<<" and column "<<guess.col+1<<endl;
        }
        else
        {
...
void SetUpBoards(Player& player) //?
{
    ...
    for(int i=0; i<NUM_SHIPS; i++)
    {
        ...
        do
        {
            ...
            if(!isValidPlacement)
            {
                cout<<endl<<"That was not a valid placement. Please, try again."<<endl<<endl;
            }
...
ShipPositionType GetBoardPosition()
{
    ...
    rowInput = GetCharacter("Input a row (A - J): ", INPUT_ERROR_STRING, validRowInputs, BOARD_SIZE, CC_UPPER_CASE);
    colInput = GetInteger("Input a column (1 - 10)", INPUT_ERROR_STRING, validColumnsInputs, BOARD_SIZE);
...
ShipOrientationType GetShipOrientation()
{
    ...
    char input =  GetCharacter("Choose orientation (H) for horizontal or (V) for vertical: ", INPUT_ERROR_STRING, validInput, 2, CC_UPPER_CASE);
...
PlayerType GetPlayer2Type()
{
    ...
    int input = GetInteger("Who would you like to play against?\n 1.Human\n 2.AI\n\n WhatIsYourChoice? ", INPUT_ERROR_STRING, validInputs, 2);

    if(input == 1) return PT_HUMAN;
    else return PT_AI;
}
